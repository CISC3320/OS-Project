...
public class os{
    ...
    //to use only when swappign out a blocked job
    static Job jobBeingSwappedIn=null;
    static Job jobBeingSwappedOut=null;
    ...
    static final int timeslice=21;
    //static final int maxWithSamePriority=2;
    //static final int[] priorityInQueue = new int[5];
    //for gathering statistics of jobs in IO queue
    static int totalBlockedRequests=0;
    static int totalIORequests=0;
    static int totalSwappedOutRequests=0;
    
    //to use only when swapping in a blocked job
    static boolean blockedJobRequestedSwapIn=false;
	...
	static void Drmint(int []a, int []p){
        ...
        if(jobMovingToMemory){
            ...
            if(blockedJobSwapInProgress){
                ...
                job=jobBeingSwappedIn;
                jobBeingSwappedIn=null;
            }
            cpuQueue.add(job);
            ...
            if(blockedJobSwapInProgress){
                ...
                System.out.println("JOB SWAPPED OUT, BLOCKED JOB CAN NOW SWAP IN");
            }
            else{
                System.out.println("SWAP OUT COMPLETE");
                Job job=jobBeingSwappedOut;
                jobBeingSwappedOut=null;
        ...
        if(blockedJobRequestedSwapIn && !jobMovingOutOfMemory && !jobMovingToMemory){
            System.out.println("BLOCKED JOB PREPARING TO SWAP OUT");
            blockedJobRequestedSwapIn=false;
            blockedJobSwapInProgress=true;
            jobMovingToMemory=true;
            moveJobOutOfMemory(jobRemovedFromMemory); 
        }        
        else if(jobsInDrum.size()>0)
	...
	static void swapToMemory(Job jobInDrum, int block){
        ...
        FreeSpaceEntry.deleteEntry(jobInDrum.block, jobInDrum.jobSize, freeSpaceTable);
		sos.siodrum(jobInDrum.jobNumber, jobInDrum.jobSize, block, 0);
	}
    
    //finds total amount of jobs on diskIO that are swapped out, blocked, and total IO requests
    //needed for algorithm in tryMovingJobOutOfMemoery
    static void gatherIOStatistics(){
        ...
        System.out.println("totalIORequests: "+totalIORequests+" totalBlockedRequests: "+totalBlockedRequests+" totalSwappedOutRequests: "+ totalSwappedOutRequests);  
            ...
            for(Job jobInDiskQueue: diskQueue){
                if(!jobInDiskQueue.jobSwappedOut && jobDoingIO!=jobInDiskQueue){
                    ...
                    if(jobComingOut!=null){
                        ...
                        if(!jobMovingToMemory && !jobMovingOutOfMemory){
                            blockedJobRequestedSwapIn=false;//means swap has been granted
                            jobMovingOutOfMemory=true;
                            blockedJobSwapInProgress=true;
                            moveJobOutOfMemory(jobRemovedFromMemory);
                        }
